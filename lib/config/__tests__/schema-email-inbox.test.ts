...
/**
 * `EMAIL_INBOX_ALLOWED_DOMAINS` is required and boot-asserted (issue #98): an inbox at
...
function withValue(raw: string | undefined) {
  if (raw === undefined) delete process.env.EMAIL_INBOX_ALLOWED_DOMAINS
  else process.env.EMAIL_INBOX_ALLOWED_DOMAINS = raw
  resetConfigCache()
  return parseDomain('emailInbox')
}

afterEach(() => {
  if (ORIGINAL === undefined) delete process.env.EMAIL_INBOX_ALLOWED_DOMAINS
  else process.env.EMAIL_INBOX_ALLOWED_DOMAINS = ORIGINAL
  resetConfigCache()
})
...
describe('emailInbox config domain', () => {
  ...
  it.each([
    ...
    expect(() => withValue(`good.example.com,${bad}`)).toThrow(/EMAIL_INBOX_ALLOWED_DOMAINS/)
  })
