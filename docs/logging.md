...
The default export is a Proxy that delegates directly to the Pino `Logger` instance — call methods on it, don't call it:

```ts
logger.info('Server started')
```

The underlying instance is a singleton, lazily created on first method call — there is no overhead from importing `logger` at the top of a module that never ends up calling it.
