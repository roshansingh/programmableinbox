...
| `EMAIL_INBOX_ALLOWED_DOMAINS` | Comma-separated list of domains inboxes may be created on. Each must be verified in Resend with its inbound route pointed at `POST /api/webhooks/email`. |
...
| `COMMERCIAL_ENABLED` | `false` | Enable the optional commercial/billing layer (needs Stripe keys when on) |
