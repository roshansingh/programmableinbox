...
- **Scopes**: exactly `email_inboxes:read`, `email_messages:read` and `email_inboxes:write` — validated against `API_KEY_SCOPE_SET` on creation. Every scope names its domain because `PhoneInbox` already exists in the schema and an unprefixed `inboxes:read` would either widen silently or need renaming later. `messages:delete` was retired in `prisma/migrations/*_retire_messages_delete_scope`; the prefixes landed in `*_prefix_email_scopes`. `DEFAULT_API_KEY_SCOPES` is enumerated explicitly rather than spread from the list — that is now load-bearing rather than precautionary, since a spread would hand every new key the write scope.
- **`email_inboxes:write` is the only write scope, and it covers create and update only.** Deletion is not on the external surface at all: a deleted inbox keeps its address forever (see `EmailInbox.email`), so it is irreversible and address-consuming in a way renaming is not. This is enforced by type, not policy — `createInbox`/`updateInboxForWrite` take an `InboxWriteScope`, `deleteInbox`/`deleteMessage`/`setMessageStarred` take an `OwnerScope`, only a `UserPrincipal` can produce the latter, and `lib/services/__tests__/scope.test-d.ts` asserts both directions. The two scope types carry an optional `__scope` phantom field purely to stop structural typing making `InboxWriteScope` assignable to `OwnerScope`.
