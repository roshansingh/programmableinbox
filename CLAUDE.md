...
- **Set-but-invalid always throws.** Unset + a default → the default; unset + required → throw; **set but malformed → throw, never a silent fallback**. `WEBHOOK_QUEUE_MAX_ATTEMPTS=abc` stops the server instead of quietly becoming `4`. Blank (`FOO=`) counts as unset, not as invalid.
...
**Required:** `DATABASE_URL`, `AUTH_JWT_SECRET`, `RESEND_WEBHOOK_SECRET`, `RESEND_API_KEY`, `AUTH_EMAIL_FROM`, `AUTH_EMAIL_FROM_NAME`, `EMAIL_INBOX_ALLOWED_DOMAINS`.

**Optional, with defaults:** `LOG_LEVEL` (debug in dev, info in prod), `ASYNC_WEBHOOK_PROCESSING_ENABLED` (false), `WEBHOOK_QUEUE_MAX_ATTEMPTS` (4), `WEBHOOK_QUEUE_CONCURRENCY_PER_INBOX` (5), `COMMERCIAL_ENABLED` (false), `WEBHOOK_EGRESS_ALLOW_PRIVATE_NETWORK` (false), `WEBHOOK_EGRESS_ALLOWED_HOSTS`, `HEALTHZ_DETAIL_SECRET`, `AUTOMATION_SWEEPER_SECRET`, `LLM_PROVIDER` / `LLM_API_KEY` / `LLM_MODEL` / `LLM_BASE_URL`, `EMAIL_VERIFICATION_ENABLED` (false), `EMAIL_VERIFICATION_TOKEN_TTL_MINUTES` (30), `AUTH_PASSWORD_RESET_TOKEN_TTL_MINUTES` (30), `MCP_ENABLED` (false), `MCP_ALLOWED_ORIGINS` (empty), `MCP_RATE_LIMIT_MAX_REQUESTS` (120), `MCP_RATE_LIMIT_WINDOW_SECONDS` (60), `OBSERVABILITY_ENABLED` (false), and the `AUTH_RATE_LIMIT_*` / `AUTH_LOCKOUT_*` / `RATE_LIMIT_*` / `AUTH_TRUSTED_PROXY_COUNT` family (see *Auth rate limiting* below).

**Conditionally required:** `REDIS_URL` has **no default** and is required whenever `ASYNC_WEBHOOK_PROCESSING_ENABLED=true` **or** `AUTH_RATE_LIMIT_ENABLED` is on (it defaults to on) — `assertConfig()` refuses to start without it, naming both reasons. A `redis://localhost:6379` fallback was removed deliberately: on a host where the variable was left unset it silently dialled a Redis that either did not exist, or belonged to another service on the same box, so two deployments shared a queue and a rate limiter. `config.redis.url` is therefore `string | null`, and every path that needs a connection goes through `requireRedisUrl()`, which throws naming the variable.

`EMAIL_LINK_SIGNING_SECRET` and `APP_BASE_URL` follow the same pattern: both `null` by default, both required whenever `EMAIL_VERIFICATION_ENABLED=true`, both asserted at boot, and both reached through `requireEmailVerification()`. `APP_BASE_URL` is operator configuration rather than the request's `Host` header on purpose — deriving the link origin from the request hands anyone who controls `Host` (or `X-Forwarded-Host`, behind a proxy that forwards it unvalidated) the domain that appears in the victim's email.

`EMAIL_LINK_SIGNING_SECRET` was renamed from `EMAIL_VERIFICATION_SECRET` and signs **every** emailed link — verification and password reset both — not just verification. The rename is breaking and has no fallback: a deployment still setting `EMAIL_VERIFICATION_SECRET` fails `assertConfig()` at boot naming `EMAIL_LINK_SIGNING_SECRET`, rather than silently starting with signing disabled. `EMAIL_VERIFICATION_TOKEN_TTL_MINUTES` and `AUTH_PASSWORD_RESET_TOKEN_TTL_MINUTES` are both whole minutes, default 30, validated by `assertConfig()` — a duration string like `30m` fails at boot rather than being coerced. The verification TTL was previously a hardcoded 24-hour constant with no env var at all, so on an upgraded deployment that leaves it unset, the window is now 30 minutes, not 24 hours — a real behavior change, not just a config rename.
...
**`EMAIL_INBOX_ALLOWED_DOMAINS` is required and asserted at boot.** A comma-separated list of the domains an inbox may be created at, parsed by the `emailInbox` schema in `lib/config/schema.ts` into a validated, de-duplicated `string[]`. Every entry must be verified in Resend *and* have its inbound route pointed at `POST /api/webhooks/email`: mail only ever reaches us for those domains, so an inbox anywhere else is unroutable by construction — created, listed, looking live in the UI, and unable to receive a single message. `assertConfig()` refuses to start without at least one valid entry, so a misconfigured deployment fails loudly at boot rather than serving a creation form that cannot work. Matching is exact — `pibx.dev` does not admit `evil.pibx.dev`. Reaches the browser as `config.emailInboxDomains` on `GET /api/app/auth/me` (see Client-visible config below), never as a `NEXT_PUBLIC_*` var, which would need a rebuild rather than a restart to change.

`OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_EXPORTER_OTLP_HEADERS`, and `OTEL_SERVICE_NAME` follow the same conditional-requirement pattern as `EMAIL_LINK_SIGNING_SECRET`/`APP_BASE_URL`: `null`/default by default, and the first two are required and asserted at boot whenever `OBSERVABILITY_ENABLED=true`. `ee/observability/init.ts` never reads the endpoint or headers *values* back out of `config` — those two are read directly from `process.env` by the OTel exporters, so `config.observability`'s role for them is purely boot-time validation. `OTEL_SERVICE_NAME` is the exception: `config.observability.serviceName` (defaulted to `programmableinbox`) is read back out of `config` and passed explicitly to both `registerOTel()` and the log transport's `resourceAttributes`, so it has a real runtime role, not just a validation one.
...
### Auth (httpOnly session cookie for the dashboard; API keys still use a header)

- **Client**: the dashboard session is a JWT carried in an httpOnly, `Secure`, `SameSite=Strict` cookie (`SESSION_COOKIE_NAME` in `lib/auth-server.ts`) — never in `localStorage` and never attached by client JS. `login`/`register` set it via `setSessionCookie()`; `apiClient` sends it automatically because every request goes out with `credentials: 'include'`. On a 401 the client redirects to `/auth/login` unless already on an auth page — there is no client-held token to clear, since the browser (not `apiClient`) owns the cookie. `POST /api/app/auth/logout` (`withUser`, `allowUnverified: true`) clears the cookie server-side; both `user-menu.tsx` and `verify-email-notice.tsx` `await` it before navigating, in a `try/finally`, so a failed logout request still ends in a full navigation to `/auth/login` rather than stranding the user on the current page.
- **Server**: route handlers do not authenticate themselves. Each is wrapped in one of three tagged wrappers from `lib/auth/with-auth.ts`, which resolves the credential and passes a principal in:
  - `withUser` — session cookie only, via `resolveUserPrincipalFromToken`. An API key is rejected without a lookup.
  - `withApiKey({ scopes })` — API key only (`Authorization: Bearer sk_live_...`), via `resolveApiKeyPrincipal`. A session cookie is rejected without verification, and the declared scopes are checked before the handler runs.
...
- **Credential type is decided by which channel it arrives on (cookie vs. `Authorization` header), not by trying one and falling back to the other.** The superseded `resolveAuthContext` verified a JWT first and fell back to an API-key lookup on the same header value — the RFC 8725 §2.8 Cross-JWT Confusion class. Session and API-key credentials now travel in different places entirely, which forecloses that confusion at the transport level rather than by pattern-matching the string.
- **`POST /api/app/auth/login` and `POST /api/app/auth/register` require `Content-Type: application/json`** (415 otherwise), checked before `request.json()` runs. `text/plain` is one of the three CORS-safelisted content types, so without this check a cross-origin page could POST credentials as a browser "simple request" — no preflight — and have the resulting `Set-Cookie` silently accepted by the victim's browser (`SameSite=Strict` governs when a cookie is later *sent*, not whether the browser may *store* one set by a request it made itself with `credentials: 'include'`). Requiring the header forces a real preflight for any cross-origin caller.
...
**`X-Forwarded-For` trust model.** Caddy is the only public ingress (`deploy/docker-compose.yml` publishes ports on `caddy` alone; `app` is internal-only) and its `reverse_proxy` *appends* the TCP peer to any inbound XFF. The chain is therefore `[...client-forged..., real IP]` and we take the entry at `length − AUTH_TRUSTED_PROXY_COUNT` — **rightmost by default, never `split(',')[0]`**, which anyone could forge to mint a fresh budget per request. Set `AUTH_TRUSTED_PROXY_COUNT=2` for a CDN in front of Caddy. IPv6 buckets by /64 (with `::` expanded first), since end users routinely get a whole /64.

When no trustworthy address can be derived — header absent, chain shorter than the declared hop count, or `AUTH_TRUSTED_PROXY_COUNT=0` — `getClientIp` returns `null` and **per-IP limiting is skipped**, with a throttled warning. It does *not* fall back to a shared `unknown` bucket: that would put every user of a proxy-less deployment (or `npm run dev`) into one 20-per-5-minutes login budget, which is a self-inflicted auth outage rather than a conservative default. Per-account limiting and lockout are unaffected. `X-Real-IP` is deliberately not a fallback — if XFF is absent there is no evidence a trusted proxy was involved, so it is exactly as forgeable.
...
- **Creation** (`POST /api/app/apiKeys`): Returns full `sk_live_*` prefixed key exactly once — user must copy/save it immediately. Under `app/`, not `v1/`: there is no `apiKeys` route in the v1 tree at all, so a key can only be minted from a dashboard session. Guard 1 is now an allowlist rather than an emptiness check — `/api/v1` may mutate only at `emailInbox` POST and `emailInbox/[id]` PATCH/DELETE, and guard 1b fails if any allowlisted handler stops existing. No collection-level DELETE: that would be a bulk destroy of every address an organization holds
...
- **Scopes**: exactly `email_inboxes:read`, `email_messages:read`, `email_inboxes:create`, `email_inboxes:update` and `email_inboxes:delete` — validated against `API_KEY_SCOPE_SET` on creation. Every scope names its domain because `PhoneInbox` already exists in the schema and an unprefixed `inboxes:read` would either widen silently or need renaming later. `messages:delete` was retired in `prisma/migrations/*_retire_messages_delete_scope`; the prefixes landed in `*_prefix_email_scopes`. `DEFAULT_API_KEY_SCOPES` is enumerated explicitly rather than spread from the list — that is now load-bearing rather than precautionary, since a spread would hand every new key the write scope.
- **The three mutating scopes are granular because they are not equally expensive to confuse.** Create and update are recoverable; delete is not. `deleteInbox` soft-deletes the inbox and its messages (F8) so the *data* survives, but `EmailInbox.email` is a plain unique index — not partial on `deletedAt IS NULL` — so the *address* is retired permanently, and there is no restore path in this codebase.
- **Three scope types keep them apart at compile time, not just at the route declaration.** `createInbox`/`updateInboxForWrite` take an `InboxWriteScope`; `deleteInbox` takes an `InboxDeleteScope`; `deleteMessage`/`setMessageStarred` keep `OwnerScope` and stay unreachable from any key. All three carry an optional `__scope` phantom field, without which they are structurally identical and freely interchangeable — `scope.test-d.ts` fails in four places if it is removed. Create and update deliberately *share* a type: confusing those two is recoverable, which is the whole basis for the split.
- **`toInboxWriteScope` and `toInboxDeleteScope` are thin wrappers over one private `resolveInboxScope`.** Same resolution, different result types. A boolean argument returning a union would put the decision back at the call site, which is what the types exist to avoid.
- **Pre-rename scopes are still accepted, for one release.** `LEGACY_SCOPE_ALIASES` maps `inboxes:read`/`messages:read` forward at the two comparison sites (`withApiKey`, and `authorize` in `lib/mcp/tools.ts`), because the migration and the deploy are not atomic and every request in that window would otherwise 403. The table is deliberately read-only — no legacy name resolves onto the write scope, since a rename must not be a privilege grant. Delete it and `resolveScope` one release after the migration has run.
...
Off unless `EMAIL_VERIFICATION_ENABLED=true`, in which case signup still returns a session token but **every `withUser` route returns 403 `Email verification required` until the address is proven** — a soft gate, not a login block.

- **The token is not a session token, by three independent barriers.** It is signed with `EMAIL_LINK_SIGNING_SECRET` (never `AUTH_JWT_SECRET`), carries `{ purpose: 'email_verify', userId, email }` where a session token carries only `{ userId }`, and `lib/auth/__tests__/verification-token.test.ts` asserts the boundary in both directions. `verifyToken` additionally rejects **any** payload carrying a `purpose` claim. This matters more than usual because a verification link travels by email — it lands in mail provider logs, corporate link scanners and browser history — so reintroducing the RFC 8725 §2.8 Cross-JWT Confusion class here would be strictly worse than the `resolveAuthContext` instance this repo already removed.
...
Shares both the `EMAIL_VERIFICATION_ENABLED` flag and its signing key: reset
links and verification links are both signed with `EMAIL_LINK_SIGNING_SECRET`. **The
...
- **Domain allowlist** — `EMAIL_INBOX_ALLOWED_DOMAINS` (above). Unconfigured → 503; wrong domain → 400.
...
**A malformed `email` on `PATCH` is a 400, not the 409.** `updateInboxForWrite` judges syntax before it compares against the stored address, because `parseInboxAddress` answers `null` both for "malformed" and for "a different address" — folding the two together reported `not-an-address` as *"the address of an inbox cannot be changed"*, which disagreed with `POST`'s 400 for the identical string and told a caller who mistyped an address they were not changing that the field is frozen. The ordering is safe rather than an oracle: the owner-scoped row is resolved first, so neither status is reachable without mutation authority.
...
- **HMAC validation**: `x-webhook-signature` + `x-webhook-timestamp` headers, verified against `RESEND_WEBHOOK_SECRET`, with a 5-minute replay window (`validateSignature`). Uses `crypto.timingSafeEqual`.
...
`POST /api/mcp` exposes the read-only email surface over MCP, authenticated with an existing `sk_live_` API key. Off unless `MCP_ENABLED=true`, in which case the route 404s — a feature that is off should not advertise that it exists.
...
- **MCP exposes create and update but never delete** — the one deliberate asymmetry with `/api/v1`, which does expose `DELETE` on `email_inboxes:delete`. A REST client deletes because someone wrote code to; a tool call can be chosen by a model reading an attacker-controlled message body. Mechanically: there is no `toInboxDeleteScope` call anywhere in `lib/mcp/`, so the compiler will not let a tool reach `deleteInbox`.
- **Every read tool is annotated `readOnlyHint: true`; the two write tools are not.** The spec defaults are `destructiveHint: true` and `openWorldHint: true`, so omitting annotations advertises a tool as destructive, and clients gate confirmation prompts on it. `create_inbox` is `idempotentHint: false` and `update_inbox` is `true` — a client retrying on timeout uses that to decide whether a second attempt is safe, and calling create twice claims two addresses.
...
Entries are **validated and canonicalised at boot**, not at request time: each must parse to a comparable `URL.origin`, and anything that does not — a bare host, `localhost:4000` (which `new URL` accepts with an *empty* host), or a non-special scheme like `chrome-extension://` whose origin serialises as the string `"null"` — fails `assertConfig()` naming the variable. Dropping them silently, which is what the first cut did, is the worst option available: `MCP_ALLOWED_ORIGINS=app.example.com` parses, contributes nothing, and 403s every request it was written to admit with no error to grep for. `normalizeOrigin` in `lib/config/primitives.ts` is the single definition of "is this an origin", used by both the schema and `checkOrigin`, so config cannot accept a value the comparison then ignores.
...
flag, `OBSERVABILITY_ENABLED` (default `false`), means it's off even on an EE
build until an operator turns it on — not tied to `COMMERCIAL_ENABLED`, since a
...
Every other variable the app needs in order to boot — `AUTH_JWT_SECRET`, `RESEND_WEBHOOK_SECRET`,
`HEALTHZ_DETAIL_SECRET`, `AUTOMATION_SWEEPER_SECRET`, `AUTH_*`, `EMAIL_INBOX_ALLOWED_DOMAINS` — is
...
`AUTH_JWT_SECRET=test-jwt-secret` sat one character under the 16-char floor `lib/config`
