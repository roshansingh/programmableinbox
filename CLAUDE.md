...
- **Set-but-invalid always throws.** Unset + a default → the default; unset + required → throw; **set but malformed → throw, never a silent fallback**. `WEBHOOK_QUEUE_MAX_ATTEMPTS=abc` stops the server instead of quietly becoming `3`. Blank (`FOO=`) counts as unset, not as invalid.
